...
// bingung banh masih belum bisa
function displayProfile() {
  var card = document.createElement("div");
  card.classList.add("card");

  var person = dummyData[0]; // Ambil data dari index pertama (0)
  ...
  if (person.bodyType) {
    profileContent += `<p><strong>Bentuk Fisik:</strong> ${person.bodyType}</p>`;
  }
  if (person.skinColor) {
    profileContent += `<p><strong>Warna Kulit:</strong> ${person.skinColor}</p>`;
  }
  if (person.hairColor) {
    profileContent += `<p><strong>Warna rambut:</strong> ${person.hairColor}</p>`;
  }
  if (person.weight) {
    profileContent += `<p><strong>Berat Badan:</strong> ${person.weight}</p>`;
  }
  if (person.height) {
    profileContent += `<p><strong>Tinggi Badan:</strong> ${person.height}</p>`;
  }

  profileContent += `</div>`; // Close profile-body
